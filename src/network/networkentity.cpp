...
void NetworkEntity::addConnection(QString ip, int port, int type)
{
    ...
    ConnectionInfo info(ip, port, true);
...
void NetworkEntity::newData()
{
    ...
    //if (type == PROTOCOL_TYPE_TEST) {
        qDebug() << "New protocol!" << "Socket index: " << protocol->getId();
    //}
...
void NetworkEntity::newNodeInfo_slot(NodeInfoMessage message, int id)
{
    emit newNodeInfo_signal(message, id);
}

void NetworkEntity::newPingReply_slot(PingReply message, int id)
{
